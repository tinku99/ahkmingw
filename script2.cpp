...
//#ifndef _MSC_VER
//#include "MSC_HEADERS/exception.h"
//#endif
	...
	, char *aImageFile, bool aSplashImage)
{
	...
	if (!*aTitle) // Provide default title.
		aTitle = const_cast<char*>((g_script.mFileName && *g_script.mFileName) ? g_script.mFileName : "");
...
ResultType Line::Transform(char *aCmd, char *aValue1, char *aValue2)
{
	...
	case TRANS_CMD_ASC:
		if (*aValue1)
			return output_var.Assign((UCHAR)*aValue1); // Cast to UCHAR so that chars above Asc(128) show as positive.
		else
			return output_var.Assign();
...
// This is because an interrupting thread usually changes the values to something inappropriate for this thread.
{
	...
	{ // Always do the first iteration so that at least one check is done.
		...
		{
		case ACT_WINWAIT:
			#define SAVED_WIN_ARGS const_cast<char*>(SAVED_ARG1), const_cast<char*>(SAVED_ARG2), const_cast<char*>(SAVED_ARG4), const_cast<char*>(SAVED_ARG5)
...
ResultType Line::ControlGetPos(char *aControl, char *aTitle, char *aText, char *aExcludeTitle, char *aExcludeText)
{
	...
	if (output_var_x && !output_var_x->Assign((long long int)(child_rect.left - parent_rect.left)))
		return FAIL;
	if (output_var_y && !output_var_y->Assign((long long int)(child_rect.top - parent_rect.top)))
		return FAIL;
	if (output_var_width && !output_var_width->Assign((long long int)(child_rect.right - child_rect.left)))
		return FAIL;
	if (output_var_height && !output_var_height->Assign((long long int)(child_rect.bottom - child_rect.top)))
		return FAIL;
...
// if we succeed here.
{
	...
	LVITEM &local_lvi = (is_win9x ? (*(LVITEM*)p_remote_lvi) : lvi_for_nt); // Local is the same as remote for Win9x.
...
ResultType Line::WinGetPos(char *aTitle, char *aText, char *aExcludeTitle, char *aExcludeText)
{
	...
	if (output_var_x)
		if (target_window)
		{
			if (!output_var_x->Assign((long long int)(rect.left)))  // X position
				result = FAIL;
		}
		else
	...
	if (output_var_y)
		if (target_window)
		{
			if (!output_var_y->Assign((long long int)(rect.top)))  // Y position
				result = FAIL;
		}
		else
	...
	if (output_var_width) // else user didn't want this value saved to an output param
		if (target_window)
		{
			if (!output_var_width->Assign((long long int)(rect.right - rect.left)))  // Width
				result = FAIL;
		}
		else
	...
	if (output_var_height)
		if (target_window)
		{
			if (!output_var_height->Assign((long long int)(rect.bottom - rect.top)))  // Height
				result = FAIL;
		}
		else
...
// is based.
{
	...
	case SYSGET_CMD_MONITORWORKAREA:
		...
		output_var_left->Assign((long long int)(monitor_rect.left));
		output_var_top->Assign((long long int)(monitor_rect.top));
		output_var_right->Assign((long long int)(monitor_rect.right));
		output_var_bottom->Assign((long long int)(monitor_rect.bottom));
		return OK;
...
fast_end:
		...
		if (!aIsPixelGetColor)
		{
			if (output_var_x && !output_var_x->Assign((long long int)((aLeft + i%screen_width) - rect.left)))
				return FAIL;
			if (output_var_y && !output_var_y->Assign((long long int)((aTop + i/screen_width) - rect.top)))
				return FAIL;
		}
	...
	if (output_var_x && !output_var_x->Assign((long long int)(xpos - rect.left)))
		return FAIL;
	if (output_var_y && !output_var_y->Assign((long long int)(ypos - rect.top)))
		return FAIL;
	// Since above didn't return:
	return g_ErrorLevel->Assign(ERRORLEVEL_NONE); // Indicate success.
}
	...
	, int aX, int aY, double aTimeout, char *aDefault)
{
	...
	if (!*aTitle)
		...
		aTitle = const_cast<char*>((g_script.mFileName && *g_script.mFileName) ? g_script.mFileName : NAME_PV);
...
// Returns OK or FAIL.
{
	...
	if (output_var_x) // else the user didn't want the X coordinate, just the Y.
		if (!output_var_x->Assign((long long int)(point.x - rect.left)))
			return FAIL;
	if (output_var_y) // else the user didn't want the Y coordinate, just the X.
		if (!output_var_y->Assign((long long int)(point.y - rect.top)))
			return FAIL;
...
ResultType Line::Drive(char *aCmd, char *aValue, char *aValue2) // aValue not aValue1, for use with a shared macro.
{
	...
	case DRIVE_CMD_UNLOCK:
		return g_ErrorLevel->Assign(DriveLock((DWORD)*aValue, (drive_cmd == DRIVE_CMD_LOCK) ? ERRORLEVEL_NONE : ERRORLEVEL_ERROR)); // Indicate success or failure.
	...
	case DRIVE_CMD_EJECT:
		...
		if (!*aValue) // When drive is omitted, operate upon default CD/DVD drive.
		{
			...
			return g_ErrorLevel->Assign((DWORD)(error ? ERRORLEVEL_ERROR : ERRORLEVEL_NONE)); // Indicate success or failure.
		}
	...
	case DRIVE_CMD_LABEL: // Note that is is possible and allowed for the new label to be blank.
		...
		return g_ErrorLevel->Assign((DWORD)(SetVolumeLabel(path, aValue2) ? ERRORLEVEL_NONE : ERRORLEVEL_ERROR));
...
ResultType Line::DriveGet(char *aCmd, char *aValue)
{
	...
	if (drive_get_cmd == DRIVEGET_CMD_SETLABEL) // The is retained for backward compatibility even though the Drive cmd is normally used.
	{
		...
		return g_ErrorLevel->Assign((DWORD)(SetVolumeLabel(path, new_label) ? ERRORLEVEL_NONE : ERRORLEVEL_ERROR));
	}
...
{
	char *target_label = const_cast<char*>(aIsDereferenced ? ARG1 : RAW_ARG1);
...
VarSizeType BIV_MMM_DDD(char *aBuf, char *aVarName)
{
	...
	case 'M': format_str = const_cast<char*>(aVarName[5] ? "MMMM" : "MMM"); break;
	case 'D': format_str = const_cast<char*>(aVarName[5] ? "dddd" : "ddd"); break;
	}
...
{
	char *type = const_cast<char*>(g_os.IsWinNT() ? "WIN32_NT" : "WIN32_WINDOWS");
	if (aBuf)
		strcpy(aBuf, type);
...
VarSizeType BIV_LoopFileFullPath(char *aBuf, char *aVarName)
{
	...
	char *full_path = const_cast<char*>(g.mLoopFile ? g.mLoopFile->cFileName : "");
	if (aBuf)
...
{
	char *str = const_cast<char*>(g.mLoopRegItem ? g.mLoopRegItem->subkey : "");
	if (aBuf)
		strcpy(aBuf, str);
	return (VarSizeType)strlen(str);
}
...
VarSizeType BIV_LoopRegName(char *aBuf, char *aVarName)
{
	...
	char *str = const_cast<char*>(g.mLoopRegItem ? g.mLoopRegItem->name : "");
	if (aBuf)
		strcpy(aBuf, str);
	return (VarSizeType)strlen(str);
}
...
{
	char buf[64];
	char *target_buf = const_cast<char*>(aBuf ? aBuf : buf);
	*target_buf = '\0'; // Set default.
...
{
	char *str = const_cast<char*>(g.mLoopReadFile ? g.mLoopReadFile->mCurrentLine : "");
	if (aBuf)
		strcpy(aBuf, str);
	return (VarSizeType)strlen(str);
}
...
{
	char *str = const_cast<char*>(g.mLoopField ? g.mLoopField : "");
	if (aBuf)
		strcpy(aBuf, str);
	return (VarSizeType)strlen(str);
}
...
{
	char *name = const_cast<char*>(g.CurrentFunc ? g.CurrentFunc->mName : "");
	if (aBuf)
		strcpy(aBuf, name);
...
{
	char *name = const_cast<char*>(g.CurrentLabel ? g.CurrentLabel->mName : "");
	if (aBuf)
		strcpy(aBuf, name);
	return (VarSizeType)strlen(name);
}
	...
	, int aOffset[], int aNumberOfIntsInOffset)
{
	...
	char *replacement = const_cast<char*>((aParamCount > 2) ? ExprTokenToString(*aParam[2], repl_buf) : "");
...
void BIF_GetKeyState(ExprTokenType &aResultToken, ExprTokenType *aParam[], int aParamCount)
{
	...
	char *mode = const_cast<char*>((aParamCount > 1) ? ExprTokenToString(*aParam[1], mode_buf) : "");
	KeyStateTypes key_state_type;
...
void BIF_WinExistActive(ExprTokenType &aResultToken, ExprTokenType *aParam[], int aParamCount)
{
	...
	for (int j = 0; j < 4; ++j) // For each formal parameter, including optional ones.
		param[j] = const_cast<char*>((j >= aParamCount) ? "" : ExprTokenToString(*aParam[j], param_buf[j]));
...
// pointers, or as two 32 bit halves for some functionsÖ
{
...
// Author: RegisterCallback() was created by Jonathan Rennison (JGR).
{
	...
	char *options = const_cast<char*>((aParamCount < 2) ? "" : ExprTokenToString(*aParam[1], options_buf));
...
// 3: (FUTURE): Possible for use with LV_FindItem (though I think it can only search item text, not subitem text).
{
	...
	if (mode_is_count)
	{
		options = const_cast<char*>((aParamCount > 0) ? omit_leading_whitespace(ExprTokenToString(*aParam[0], buf)) : "");
		if (*options)
		{
	...
	options = const_cast<char*>((aParamCount > 1) ? ExprTokenToString(*aParam[1], buf) : "");
...
// In Add/Insert mode, if there are no text fields present, a blank for is appended/inserted.
{
	...
	char *options = const_cast<char*>((aParamCount > 0) ? ExprTokenToString(*aParam[0], buf) : "");
...
// There are also some special modes when only zero or one parameter is present, see below.
{
	...
	char *options = const_cast<char*>((aParamCount > 1) ? ExprTokenToString(*aParam[1], buf) : "");
...
//    1: ID of item to delete (if omitted, all items are deleted).
{
	...
	if (add_mode) // TV_Add()
	{
		...
		options = const_cast<char*>((aParamCount > 2) ? ExprTokenToString(*aParam[2], buf) : "");
	}
	else // TV_Modify()
	{
